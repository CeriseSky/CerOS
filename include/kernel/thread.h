...
typedef struct {
  ...
  int requestAge; // Number of quantums the thread has waited to execute
  int present; // useful for fixed size arrays. 1 if the thread exists
  int active;
  int sleepLeft;  // Number of ticks that the thread is to sleep for
} thread_t;

// changes output to the context of the thread that has been
// waiting the most quantums to execute. If a thread was not chosen,
